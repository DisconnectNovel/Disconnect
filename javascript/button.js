...
function updateText_d2_At_school() {
    ...
    if (talked_to_Mona) {
        ...
        txt = "De retour à l'université le lendemain, Aaron voit Mona de l'autre côté de la cour. Il croise son regard, qui semble vidé d'émotion. Un poster flamboyante présentant l'équipe de basket détourne son attention. « Recherche de nouvelles recrues talentueuses ! » tonne l'affiche. Aaron sent une bouffée d'excitation s'emparer de lui. Il pensait justement rejoindre l'équipe de basket; il en avait fait plus jeune à un niveau relativement élevé et appréciait beaucoup ce sport. L'affiche invite à venir à l'un des entraînements de midi pour faire connaissance et participer à l'échauffement.<br /><br />";
...
function updateText_d2_At_health_insurance() {
    ...
    if (ran) {
        text += "  - Les données récoltées à partir de l'application que Aaron avait utilisée pour aller courir indiquent que ses performances sont assez faibles. Le rythme cardiaque était particulièrement élevé lors de la course, indicateur de mauvaise santé physique.<br />";
    }
    else {
    ...
    text += "<br />L'agent avertit Aaron que s'il souhaite vraiment obtenir une assurance santé, le prix sera nettement plus élevé pour prendre en compte les facteurs de risque. Aaron, choqué, se rend compte qu'il n'est pas capable de payer une telle somme et qu'il doit abandonner l'idée de faire du basket. Une fois remis du choc, il sent une colère bouillonnante monter en lui: « Si je veux me faire assurer, c'est justement pour pouvoir faire du sport et par conséquent diminuer mes risques, pourquoi vous ne me laissez pas le faire ! » fulmine-t-il en tapant du poing sur la table. La sécurité est appelée mais Aaron sort en coup de vent avant qu'elle n'ait eu le temps d'arriver.<br /><br />Une caméra à l'intérieur de la pièce a enregistré la scène.";
...
function updateText_d2_At_home() {
    ...
    if (stopped_when_Mona_sad) {
        text+= "Il reçoit alors un message bizarre d'un ami de l'université qui l'aurait vu en companie de Mona. Celui-ci lui demande pourquoi il perd son temps avec une fille comme ça et l'avertit qu'il risque de gâcher sa vie s'il continue à fréquenter cette fille. Aaron est surpris que des gens s'intéressent à ce point à ses fréquentations.";
    }
